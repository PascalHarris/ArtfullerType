#!/bin/bash
# Builds DodgerType and deploys it to both test images. The 800K floppy
...
FLOPPY_IMAGE_NAME="DodgerType 800K"
BIN="app/build/DodgerType.bin"
GUIDE="doc/START_HERE.md"
GUIDE_NAME="START_HERE.md"
WRAPPED_FLOPPY_TEMP="vmac/.DodgerType_Floppy_wrapped.img"

echo "Building..."
cmake --build app/build

echo "Wrapping the 800K floppy image in a DiskCopy 4.2 header..."
python3 make_diskcopy_image.py "$FLOPPY_IMAGE" "$WRAPPED_FLOPPY_TEMP" "DodgerType"

echo "Deploying to 20MB image..."
hmount "$HD_IMAGE"
hdel DodgerType 2>/dev/null || true
hcopy -m "$BIN" :DodgerType
